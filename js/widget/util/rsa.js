...
(function($, undefined){
    ...
    rsa.biFromDecimal = function(s){
        ...
        if (i == s.length) {
            ...
            i += fgl;
            while (i < s.length) {
                result = rsa.biAdd(rsa.biMultiply(result, lr10), rsa.biFromNumber(Number(s.substr(i, dpl10))));
                i += dpl10;
            }
            result.isNeg = isNeg;
        }
    ...
    rsa.biToHex = function(x){
        ...
        for (var i = rsa.biHighIndex(x); i > -1; --i) {
    ...
    rsa.biShiftLeft = function(x, n){
        ...
        var result = new BigInt();
        rsa.arrayCopy(x.digits, 0, result.digits, digitCount, result.digits.length - digitCount);
    ...
    rsa.biShiftRight = function(x, n){
        var digitCount = Math.floor(n / bitsPerDigit);
        var result = new BigInt();
        rsa.arrayCopy(x.digits, digitCount, result.digits, 0, x.digits.length - digitCount);
        var bits = n % bitsPerDigit;
    ...
    var RSAKeyPair = rsa.RSAKeyPair = function(encryptionExponent, decryptionExponent, modulus){
        var $dmath = rsa;
