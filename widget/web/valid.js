...
(function($, UI, undefined){
    ...
    Valid.prototype = {
        ...
        add: function(els){
            var self = this;
            els = $(els);
            for (var i = 0, len = els.length, options, el; i < len; i++) {
                //已经存在
                ...
                try {
                    options = $.extendIf(eval('(' + (el.data('valid') || el.attr('valid') || '{}') + ')'), defaults);
                    options.defValue = val(els[i], options);
                } 
                catch (e) {
                    $.log('valid属性内容格式错误');
                }
                //追加配置
                self._elConfigs.push(options);
                //动作事件绑定
                el.bind(options.evt + '.valid', {
                    el: els[i],
                    cfg: options,
                    opt: options
